...
volume_status=$($EC2_HOME/bin/ec2-attach-volume $aws_snapshot_volume_id --region $aws_region \
  --device $aws_snapshot_device --instance $current_instance_id -O $aws_access_key_id -W $aws_access_secret_key)
...
while [[ "$completed" == "" ]]
do
    completed=$($EC2_HOME/bin/ec2-describe-volumes $aws_snapshot_volume_id --region $aws_region  -O $aws_access_key_id -W $aws_access_secret_key | grep attached)
    echo -n ". "
    sleep 3
done
echo ""
log_msg=$($EC2_HOME/bin/ec2-describe-volumes $aws_snapshot_volume_id --region $aws_region  -O $aws_access_key_id -W $aws_access_secret_key | grep attached)
log_output
...
volume_status=$($EC2_HOME/bin/ec2-describe-volumes --region $aws_region $aws_snapshot_volume_id  -O $aws_access_key_id -W $aws_access_secret_key | grep attached)
...
while [[ "$completed" == "" ]]
do
    completed=$($EC2_HOME/bin/ec2-describe-snapshots $aws_snapshot_id --region $aws_region  -O $aws_access_key_id -W $aws_access_secret_key | grep completed)
    echo -n ". "
    sleep 3
done
echo ""
log_msg=$($EC2_HOME/bin/ec2-describe-snapshots $aws_snapshot_id --region $aws_region  -O $aws_access_key_id -W $aws_access_secret_key | grep completed)
log_output
...
log_msg=$($EC2_HOME/bin/ec2-create-tags $aws_registerd_ami_id --region $aws_region \
  --tag Name="$aws_ami_description" --tag Project=$project  -O $aws_access_key_id -W $aws_access_secret_key)
log_output
